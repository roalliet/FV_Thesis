...
def calc_F_f(F, h, ax):
    ...
    F_f = (7 * (F + np.roll(F, -1, axis=ax))  - ( np.roll(F, -2, axis=ax) + np.roll(F, 1, axis=ax))) / (h * 12)
    return F_f


def calc_GFx(F, G0_xf, dx, dv, t):
    GFx = np.empty(F.shape)
...
def plot_distr(F, t):
    ...
    plt.savefig(f'swirling_deformation_figs_CD4_RK4/swirl_diff_res_{grid_size}_t_{t:.2f}.png', dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI

    plt.show()


def run_CD4_RK4(plot=False):
...
def calc_norm_evo():
    ...
    plt.title('Relative L1-norm using CD4 RK4 with respect to the gridsize')
    ...
    # Save the plot as an image file
    plt.savefig("swirling_deformation_figs_CD4_RK4/swirl_diff_L1_CD4_RK4.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
    ...
    plt.title('Relative L2-norm using CD4 RK4 with respect to the gridsize')
    plt.xlabel('number of gridpoints')
    ...
    # Save the plot as an image file
    plt.savefig("swirling_deformation_figs_CD4_RK4/swirl_diff_L2_CD4_RK4.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
    ...
    plt.title('Relative L-$\\infty$-norm using CD4 RK4 with respect to the gridsize')
    plt.xlabel('number of gridpoints')
    ...
    # Save the plot as an image file
    plt.savefig("swirling_deformation_figs_CD4_RK4/swirl_diff_L_infty_CD4_RK4.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
...
F_init, F  = run_CD4_RK4(plot=0.25)
