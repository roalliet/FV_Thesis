...
def calc_GFx(F, G0_x, dx, dv, t):
    ...
    ddg_f = (np.roll(g_f, -1, axis=0) - 2 * g_f + np.roll(g_f, 1, axis=0)) / (dv**2)
    G_f = g_f + dv**2 * (ddg_f) / 24

    #determine the first derivative from the face integrated values of f and g
    df_f = (np.roll(F_f, -1, axis=0) - np.roll(F_f, 1, axis=0)) / (dv * 2)
    dg_f = (np.roll(g_f, -1, axis=0) - np.roll(g_f, 1, axis=0)) / (dv * 2)

    #calculate the face integrated fluxes on the x-faces
    GFx = F_f * G_f - dv**2 * df_f * dg_f / 12
    return GFx
...
def calc_GFv(F, G0_v, dx, dv, t):
    ...
    ddg_f = (np.roll(g_f, -1, axis=1) - 2 * g_f + np.roll(g_f, 1, axis=1)) / (dx**2)
    G_f = g_f + dx**2 * (ddg_f) / 24

    #determine the second order derivative from the face integrated values of f and g
    df_f = (np.roll(F_f, -1, axis=1) - np.roll(F_f, 1, axis=1)) / (dx * 2)
    dg_f = (np.roll(g_f, -1, axis=1) - np.roll(g_f, 1, axis=1)) / (dx * 2)

    #calculate the face integrated fluxes on the x-faces
    GFv = F_f * G_f + dx**2 * df_f * dg_f / 12
    return GFv
...
def plot_distr(F, t):
    ...
    plt.savefig(f'swirling_deformation_figs_UP4_RK2_1stder/swirl_diff_res_{grid_size}_t_{t:.2f}.png', dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
...
def calc_norm_evo():
    ...
    # Save the plot as an image file
    plt.savefig("swirling_deformation_figs_UP4_RK2_1stder/swirl_diff_L1_UP4_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
    ...
    # Save the plot as an image file
    plt.savefig("swirling_deformation_figs_UP4_RK2_1stder/swirl_diff_L2_UP4_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
    ...
    # Save the plot as an image file
    plt.savefig("swirling_deformation_figs_UP4_RK2_1stder/swirl_diff_L_infty_UP4_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
