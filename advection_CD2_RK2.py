...
def advance_CD2_RK2(f, dx, dt):
    #determine the sum of the fluxes through the surfaces of the control volume
    f_temp = f - v * dt / (4 * dx) * (np.roll(f, -1) - np.roll(f, 1))
    f -= v * dt / (2 * dx) * (np.roll(f_temp, -1) - np.roll(f_temp, 1))
    return f


def run_CD2_RK2(grid_size):
...
def calc_norm_evo(reps):
    ...
    for exp in range(reps):
        ...
        _, f_num, _ = run_CD2_RK2(grid_size)
    ...
    plt.savefig("fitted_L1_sine_CD2_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
    ...
    # Save the plot as an image file
    plt.savefig("fitted_L2_sine_CD2_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
    ...
    # Save the plot as an image file
    plt.savefig("fitted_L_infty_sine_CD2_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
...
f_init, f, x = run_CD2_RK2(grid_size)
...
# Save the plot as an image file
plt.savefig("initial_distr_sine_CD2_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI

plt.show()


plt.plot(x, f, label='CD2 RK2')
...
# Save the plot as an image file
plt.savefig("final_distr_sine_CD2_RK2.png", dpi=300, bbox_inches='tight')  # Saves as PNG with 300 DPI
